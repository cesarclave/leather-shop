...
-- Add streams to see newly added data
-- STREAMS on DEV tables (append-only to track new inserts)
USE ROLE SYSADMIN;
CREATE STREAM RAW_DEV.RAW.ORDERS_STREAM
  ON TABLE RAW_DEV.RAW.ORDERS
  APPEND_ONLY = TRUE
  COMMENT = 'Track new order inserts in DEV';

CREATE STREAM RAW_DEV.RAW.CUSTOMERS_STREAM
  ON TABLE RAW_DEV.RAW.CUSTOMERS
  APPEND_ONLY = TRUE
  COMMENT = 'Track new customer inserts in DEV';

CREATE STREAM RAW_DEV.RAW.PRODUCTS_STREAM
  ON TABLE RAW_DEV.RAW.PRODUCTS
  APPEND_ONLY = TRUE
  COMMENT = 'Track new product inserts in DEV';

-- Query streams to see new additions
 SELECT * FROM RAW_DEV.RAW.ORDERS_STREAM;
 SELECT * FROM RAW_DEV.RAW.CUSTOMERS_STREAM;
 SELECT * FROM RAW_DEV.RAW.PRODUCTS_STREAM;
